...
const Footer = () => {
  return (
    <footer className="bg-brand-dark text-secondary border-t-2 border-brand-red">
      <div className="container mx-auto px-4 py-12">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              {["DSL & Festnetz", "Mobilfunk", "Handyservice", "WLAN-Einrichtung", "VoIP-Telefonie", "Geschäftskunden"].map((s) => (
                <li key={s}>
                  <Link to="/leistungen" className="hover:text-primary transition-colors">
                    {s}
                  </Link>
                </li>
              ))}
        ...
        <div className="mt-10 pt-6 border-t border-muted flex flex-col md:flex-row items-center justify-between gap-4">
          <img
            ...
            alt="Gefördert durch Freistaat Thüringen"
            className="h-12"
          />
          <p className="text-xs text-muted-foreground">
            © 2025 NextPhones | Alle Rechte vorbehalten
          </p>
        </div>
      </div>
    </footer>
